...
const Header = props => {
  ...
  const renderCartIcon = () => (
    <div className='cart-icon-link'>
      <Link to='/cart'>
        <button data-testid='cart' type='button' className='cart-icon-button'>
  ...
  return (
    <header className='p-4 d-flex flex-row align-items-center nav-header'>
      <Link to='/'>
        <h1 className='m-8 Logo-heading'>{restaurantName}</h1>
      </Link>
      <div className='d-flex flex-row align-items-center ms-auto'>
        <p className='mt-08 mb-0 me-2 d-none d-sm-block my-orders-text'>
          My Orders
        </p>
        <button
          type='button'
